...
public class DBManager {
    ...
    public String getProjectName(String projectIdentifier) {
        ...
        try {
            ...
            life.qbic.MyPortletUI.logger.error("SQL operation unsuccessful: " + e.getMessage());
            e.printStackTrace();
        } catch (NullPointerException n) {
            life.qbic.MyPortletUI.logger.error("Could not reach SQL database, resuming without project names.");
        }
        logout(conn);
        return res;
    }
    ...
    public int isProjectInDB(String projectIdentifier) {
        life.qbic.MyPortletUI.logger.info("Looking for project " + projectIdentifier + " in the DB");
        ...
        try {
            ...
            if (rs.next()) {
                res = rs.getInt("id");
                life.qbic.MyPortletUI.logger.info("project found!");
            }
        } catch (SQLException e) {
            life.qbic.MyPortletUI.logger.error("SQL operation unsuccessful: " + e.getMessage());
            e.printStackTrace();
    ...
    public int addProjectToDB(String projectIdentifier, String projectName) {
        ...
        if (exists < 0) {
            life.qbic.MyPortletUI.logger.info("Trying to add project " + projectIdentifier + " to the person DB");
            ...
            try (PreparedStatement statement =
                ...
                if (rs.next()) {
                    logout(conn);
                    life.qbic.MyPortletUI.logger.info("Successful.");
                    return rs.getInt(1);
                }
            } catch (SQLException e) {
                life.qbic.MyPortletUI.logger.error("SQL operation unsuccessful: " + e.getMessage());
    ...
    public boolean hasPersonRoleInProject(int personID, int projectID, String role) {
        life.qbic.MyPortletUI.logger.info("Checking if person already has this role in the project.");
        String sql =
        ...
        try {
            ...
            if (rs.next()) {
                res = true;
                life.qbic.MyPortletUI.logger.info("person already has this role!");
            }
        } catch (SQLException e) {
            life.qbic.MyPortletUI.logger.error("SQL operation unsuccessful: " + e.getMessage());
            e.printStackTrace();
    ...
    public void addPersonToProject(int projectID, int personID, String role) {
        if (!hasPersonRoleInProject(personID, projectID, role)) {
            life.qbic.MyPortletUI.logger.info("Trying to add person with role " + role + " to a project.");
            String sql =
    ...
    public int addExperimentToDB(String id) {
        ...
        if (exists < 0) {
            ...
            try (PreparedStatement statement =
                ...
                life.qbic.MyPortletUI.logger.error("Was trying to add experiment " + id + " to the person DB");
                life.qbic.MyPortletUI.logger.error("SQL operation unsuccessful: " + e.getMessage());
            }
            logout(conn);
            return -1;
        }
        life.qbic.MyPortletUI.logger.info("added experiment do mysql db");
        return exists;
    }

    private int isExpInDB(String id) {
        life.qbic.MyPortletUI.logger.info("Looking for experiment " + id + " in the DB");
        ...
        try {
            ...
            ResultSet rs = statement.executeQuery();
            if (rs.next()) {
                life.qbic.MyPortletUI.logger.info("experiment found!");
                res = rs.getInt("id");
            }
        } catch (SQLException e) {
            life.qbic.MyPortletUI.logger.error("SQL operation unsuccessful: " + e.getMessage());
    ...
    public void addPersonToExperiment(int expID, int personID, String role) {
        ...
        if (!hasPersonRoleInExperiment(personID, expID, role)) {
            life.qbic.MyPortletUI.logger.info("Trying to add person with role " + role + " to an experiment.");
            String sql =
    ...
    private boolean hasPersonRoleInExperiment(int personID, int expID, String role) {
        life.qbic.MyPortletUI.logger.info("Checking if person already has this role in the experiment.");
        String sql =
